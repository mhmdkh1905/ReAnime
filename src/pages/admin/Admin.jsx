...
export default function Admin() {
  ...
  const handleDeleteUser = async (userId) => {
    ...
    try {
      ...
      for (const movie of userMovies) await deleteMovie(movie.id);
      setMovies(movies.filter((m) => m.createdBy !== userId));
      const userScenarios = scenarios.filter((s) => s.createdBy === userId);
      for (const scenario of userScenarios) await deleteScenario(scenario.id);
    ...
    <div className="admin-page">
      <div className="admin-container">
        ...
        <div className="admin-stats">
          {[
            { icon: "👥", count: stats.users, label: "Users" },
            { icon: "🎬", count: stats.movies, label: "Movies" },
            { icon: "📝", count: stats.scenarios, label: "Scenarios" },
            { icon: "💬", count: stats.comments, label: "Comments" },
        ...
        <div className="admin-tabs">
          {["users", "movies", "scenarios", "comments"].map((tab) => (
            <button
              key={tab}
